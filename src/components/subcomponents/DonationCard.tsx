...
import { Alert, Avatar, Button, Card, Toast } from "flowbite-react";
...
export default function DonationCard({
  ...
  const displayToast = (message: string) => {
    toast(
      <div className="ml-3 text-md font-normal text-white">{message}</div>,
      {
  ...
  const onDelete = async () => {
    ...
    if (!responseList.error && !responsePost.error) {
      ...
      displayToast("Removed Successfull");
    }
  ...
  return (
    ...
    >
      <Card className="flex w-full h-full flex-col justify-between">
